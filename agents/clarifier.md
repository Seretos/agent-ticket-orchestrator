...
- **Measurement** — <does the AC measure that symptom or an internal quantity; if internal, the exact sentence the AC must gain>
...
- `symptom:` names a user-visible behaviour **and**
  `measurement: internal:<q>` ⇒ **not CLEAR**. Emit exactly one question:
  extend the AC with the symptom and demote `<q>` to a helper measurement
  *(recommended)* / proceed measuring `<q>` only.
- `chain: regression-chain:…` ⇒ **not CLEAR**, *unless* the ticket is already
  framed as a root-cause or simplification task — its AC measures the symptom
  **and** it names a non-goal along the lines of "another guard", "another
  constant", "another retry". Otherwise emit exactly one question: reframe
  this as a root-cause / simplification task with a symptom AC and the
  explicit non-goal "another guard" *(recommended)* / proceed as another
  point fix, accepting that #a and #b already did that and the symptom
  survived. This is the reframing mandate, discharged in the pass that
  detected the chain — there is no second dispatch.
...
- **`lib-python-worktree#148` shape** — thread-leak framing, AC "thread count
  ...
  `measurement: internal:live thread count`, `prior_attempts: #90,#121`,
  `chain: regression-chain:#90,#121`, ends `STATUS: NEEDS_INPUT` with the
  reframe question.
- **The ordinary shape** — symptom named, AC measures it, no predecessors →
  `measurement: symptom`, `chain: none`, ends `STATUS: CLEAR` exactly as
  before.
