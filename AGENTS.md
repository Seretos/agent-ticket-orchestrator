...
|---|---|---|---|---|
| `gatekeeper` | starts the session, not needed at the keyboard while it runs | **no — never granted, never used** | posts `## Clarification needed (gatekeeper)` on the package ticket, moves it to Question, moves to the next package | epics, capability tickets, `parent` relations, `blocked_by`/`relates_to` relations, `epic`/`regression-chain`/`pipeline-capability` labels, clarification/frame/dependency/regression-chain/capability-split/release-confirmation comments, Backlog → Planned, Backlog → Question, Question → Planned (own answered cards only) |
