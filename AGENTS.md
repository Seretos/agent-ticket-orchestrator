...
A maintainer parks a ticket in Backlog on purpose by labelling it `gatekeeper-ignore`; the gatekeeper's Step 1 passes `not_labels=["gatekeeper-ignore"]` on both `list_tickets` calls, so an ignored ticket never reaches the bundler, the clarifier, an epic or a column move — no per-step "skip if labelled" checks exist or should be added (`agent-ticket-orchestrator#37`). The gatekeeper never creates, adds or removes the label; filtering on a label missing from the repository's catalog does not fail (verified on GitHub 2026-09-21; not verified on GitLab/Azure DevOps). Two report-only `labels=` calls feed the `ignored (…)` line so a forgotten label stays visible. `run` deliberately does **not** look at the label: it only matters before Planned, and a human who moves an ignored ticket to Todo has decided.

### A capability the PR cannot execute is its own ticket, and only the automatable half blocks

Incident: `agent-project-issues` #347 carried an acceptance criterion whose test procedure needed a release-only job and a real Windows shell. The package merged `ci-green`, the release that followed failed on exactly that procedure, and each attempt burned a version number with no product defect involved (`agent-ticket-orchestrator#33`). Green CI proved nothing about a capability the PR's own run never executed.

Same three-level split as dependencies: the `clarifier` **reports** (frame key `needs_pipeline_support: none | auto:<capability> | manual:<check>`, repeatable, both statuses, detected by Protocol step 1d), the `gatekeeper` **writes** (Step 3.4: a `pipeline-capability` ticket in Backlog from the `task` template, a recut so the criterion moves to it, one `## Capability split (gatekeeper)` comment carrying a `gatekeeper:capability v1` block that makes a second pass a no-op), `run` **obeys** and is unchanged. Step 3.4 sits before Step 3.5 so the relation goes through Step 3.5's write and read-back and the comments through Step 3.7's — no second write path.

The owner's decision (question 1, option b): an `auto:` capability, one an automated job could ever run, becomes a `blocked_by` on the package, because the package's own criterion is unverifiable until it exists. A `manual:` capability writes no relation and blocks nothing — a person's check would otherwise hold the package for a step no job will ever perform, and the package ships the same night. When the clarifier finds an open ticket for the capability it reports `depends_on` for `auto:` and never for `manual:`, for the same reason.

The `pipeline-capability` label exists so the clarifier's hatch closure has something to read: `none:ci|test` with `measurement: n/a` is refused for such a ticket, since presence of a step in a workflow file is what #348 delivered. Detection stays cheap on purpose — a criterion that names none of the six shapes is `none`. No mock clarifier: the worked frames in `agents/clarifier.md` are the executable form.
...
|---|---|---|---|---|
| `gatekeeper` | starts the session, not needed at the keyboard while it runs | **no — never granted, never used** | posts `## Clarification needed (gatekeeper)` on the package ticket, moves it to Question, moves to the next package | epics, the prose half of a lane split, capability tickets, `parent` relations, `blocked_by`/`relates_to` relations, `epic`/`regression-chain`/`pipeline-capability` labels, the `lane:prose` label, clarification/frame/dependency/regression-chain/capability-split/release-confirmation comments, Backlog → Planned, Backlog → Question, Question → Planned (own answered cards only) |
...
- **Dependencies** are declared in `.claude-plugin/plugin.json` (`agent-autonomous-developer`, `agent-autonomous-prompt-engineer`, `agent-project-issues`, `agent-worktree`); the two lower plugins' floors are the first releases that carry the `process-developer` / `process-prompt-engineer` skill names; Claude Code installs/loads them with this plugin. The Codex manifest (`.codex-plugin/plugin.json`) carries no dependency field — Codex hosts install the MCPs separately.
