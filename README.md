...
| `pipeline-capability` | gatekeeper | A capability split off a package because its PR run could not execute it. |
...
"enabledPlugins": {
  ...
  "agent-autonomous-prompt-engineer@agent-marketplace": true,
...
- `agents/bundler.md`, `agents/clarifier.md` — read-only Opus subagents used by `gatekeeper`. The `bundler` sizes every ticket, caps a `collision` package at one `size: large` ticket, and proposes a `recut` in place of an epic when that cap rejects an overlapping large pair. The `clarifier` also interrogates the ticket's problem frame, writes a missing acceptance criterion, records unverified premises, detects regression chains, and reports an acceptance criterion that needs a capability the PR's own run cannot execute (`needs_pipeline_support`), which the `gatekeeper` splits into its own `pipeline-capability` ticket — blocking the package only when an automated job could run it.
